...
export default function ApprovalSetupCreatePage() {
  const { employees, addWorkflowStage } = useData()
  const router = useRouter()
  const [stageName, setStageName] = useState("")
  const [approvalMode, setApprovalMode] = useState<ApprovalMode>("sequential")
  const [assignedDesignations, setAssignedDesignations] = useState<string[]>([])
  const [assignedEmployees, setAssignedEmployees] = useState<string[]>([])
  const [employeeDropdownOpen, setEmployeeDropdownOpen] = useState(false)

  // Review types
  const [requiresAccountsReview, setRequiresAccountsReview] = useState(false)
  const [requiresMaintenanceReview, setRequiresMaintenanceReview] = useState(false)
  const [requiresCentralAccountsReview, setRequiresCentralAccountsReview] = useState(false)

  // Officers
  const [accountsOfficers, setAccountsOfficers] = useState<string[]>([])
  const [maintenanceOfficers, setMaintenanceOfficers] = useState<string[]>([])
  const [centralAccountsOfficers, setCentralAccountsOfficers] = useState<string[]>([])
  ...
  const handleSave = () => {
    ...
    addWorkflowStage({
      name: stageName,
      workflowType: "preapproval", // Default to preapproval
      assignedDesignations,
      assignedEmployees,
      approvalMode,
      order: 1, // Will be adjusted based on existing stages
